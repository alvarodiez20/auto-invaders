...
export class ShopUI {
  ...
  private getUpgradeMetaLines(upgrade: UpgradeDefinition, level: number): string[] {
    ...
    // Weapon upgrades - show current values
    if (upgrade.id === 'damage') {
      const currentDamage = this.scene.upgradeManager.getDamage();
      lines.push(`Current damage: ${currentDamage.toFixed(1)}`);
    }

    if (upgrade.id === 'fireRate') {
      const currentRate = this.scene.upgradeManager.getFireRate();
      lines.push(`Current fire rate: ${currentRate.toFixed(2)}/s`);
    }

    if (upgrade.id === 'projectileSpeed') {
      const currentSpeed = this.scene.upgradeManager.getBulletSpeed();
      lines.push(`Current speed: ${Math.round(currentSpeed)}`);
    }

    if (upgrade.id === 'critChance') {
      const critChance = level * 2;
      lines.push(`Current crit chance: ${critChance}%`);
    }

    if (upgrade.id === 'critMultiplier') {
      const critMult = 1.5 + level * 0.15;
      lines.push(`Current crit damage: ${critMult.toFixed(2)}x`);
    }

    // Economy upgrades
    if (upgrade.id === 'salvageYield') {
      const salvageBonus = level * 5;
      lines.push(`Current salvage: +${salvageBonus}%`);
    }

    // Survival upgrades
    if (upgrade.id === 'hull') {
      const save = SaveManager.getCurrent();
      lines.push(`Current max HP: ${Math.round(save.playerMaxHP)}`);
    }

    if (upgrade.id === 'stability') {
      const stabilityBonus = level * 7;
      lines.push(`Current stability: +${stabilityBonus}%`);
    }

    if (upgrade.id === 'heatCapacity') {
      const baseCapacity = 100;
      const currentCapacity = Math.round(baseCapacity * (1 + level * 0.10));
      lines.push(`Current capacity: ${currentCapacity}`);
    }

    if (upgrade.id === 'coolingRate') {
      const coolingBonus = level * 8;
      lines.push(`Current cooling: +${coolingBonus}%`);
    }
